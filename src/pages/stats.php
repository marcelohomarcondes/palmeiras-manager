...
if (!function_exists('render_table_scroll')) {
  function render_table_scroll(array $headers, array $rows, array $keys, int $colspanIfEmpty, int $maxHeightPx = 340): void {
    ...
    echo '<table class="table table-sm mb-0">';
    echo '<thead style="position:sticky; top:0; z-index:2;"><tr>';
    foreach ($headers as $hh) echo '<th>'.h((string)$hh).'</th>';
    echo '</tr></thead><tbody>';
...
if (!function_exists('pm_stats_real_participation_sql')) {
  function pm_stats_real_participation_sql(string $clubNormExpr, bool $mpHasUserId, bool $msHasUserId, bool $mxHasUserId): string {
    ...
    return "
      ...
      SELECT DISTINCT ms.match_id, ms.player_in_id AS player_id
      ...
      WHERE ms.player_in_id IS NOT NULL
        ...
        AND (
          ...
          OR
          (UPPER(TRIM(mx.away)) = $clubNormExpr AND UPPER(TRIM(COALESCE(ms.side,''))) = 'AWAY')
        )
    ";
  }
}

if (!function_exists('pm_stats_is_goalkeeper_expr')) {
  function pm_stats_is_goalkeeper_expr(string $positionExpr): string {
...
function compute_streaks(array $matches): array {
  ...
  foreach ($matches as $m) {
    ...
    foreach ($conditions as $k => $ok) {
      if ($ok) {
        if ($cur[$k]['len'] === 0) $cur[$k]['start'] = $date;
        $cur[$k]['len']++;
        if ($cur[$k]['len'] > $best[$k]['len']) {
          $best[$k] = ['len'=>$cur[$k]['len'],'start'=>$cur[$k]['start'],'end'=>$date];
        }
      } else {
        $cur[$k] = ['len'=>0,'start'=>null];
      }
    }
  }
...
$playersHasUserId          = table_exists($pdo, 'players') ? has_user_id_col($pdo, 'players') : false;
$matchPlayersHasUserId     = table_exists($pdo, 'match_players') ? has_user_id_col($pdo, 'match_players') : false;
$matchSubsHasUserId        = table_exists($pdo, 'match_substitutions') ? has_user_id_col($pdo, 'match_substitutions') : false;
$matchPlayerStatsHasUserId = table_exists($pdo, 'match_player_stats') ? has_user_id_col($pdo, 'match_player_stats') : false;

$clubNorm = "UPPER(TRIM(:club))";
$realParticipationSql = pm_stats_real_participation_sql($clubNorm, $matchPlayersHasUserId, $matchSubsHasUserId, $matchesHasUserId);
...
if ($season !== '') {
  $where[] = "CAST(m.season AS TEXT) = :season";
  $bind[':season'] = $season;
}
if ($competition !== '') {
...
$clubGF = "CASE WHEN UPPER(TRIM(m.home)) = $clubNorm THEN COALESCE(m.home_score,0) ELSE COALESCE(m.away_score,0) END";
$clubGA = "CASE WHEN UPPER(TRIM(m.home)) = $clubNorm THEN COALESCE(m.away_score,0) ELSE COALESCE(m.home_score,0) END";
$isClubInMatch = "(UPPER(TRIM(m.home)) = $clubNorm OR UPPER(TRIM(m.away)) = $clubNorm)";

// -----------------------------------------------------------------------------
// Opções de filtro
// -----------------------------------------------------------------------------
$seasonOptions = [];
$sql = "SELECT DISTINCT CAST(season AS TEXT) AS season FROM matches m "
     . ($matchesHasUserId ? "WHERE m.user_id = :user_id " : "")
     . "ORDER BY CAST(season AS INTEGER) DESC";
$st = $pdo->prepare($sql);
$params = [];
if ($matchesHasUserId) $params[':user_id'] = $userId;
$st->execute($params);
foreach (($st->fetchAll(PDO::FETCH_ASSOC) ?: []) as $r) {
  $v = trim((string)($r['season'] ?? ''));
  if ($v !== '') $seasonOptions[] = $v;
}

$competitionOptions = [];
$sql = "SELECT DISTINCT competition FROM matches m "
     . ($matchesHasUserId ? "WHERE m.user_id = :user_id " : "")
...
$st->execute($params);
foreach (($st->fetchAll(PDO::FETCH_ASSOC) ?: []) as $r) {
  $v = trim((string)($r['competition'] ?? ''));
  if ($v !== '') $competitionOptions[] = $v;
}

// -----------------------------------------------------------------------------
// Base de partidas filtradas
// -----------------------------------------------------------------------------
$sqlMatches = "
  SELECT
    m.*,
    ...
    $clubGA AS ga,
    CASE
      WHEN UPPER(TRIM(m.home)) = $clubNorm THEN COALESCE(m.away,'')
      ELSE COALESCE(m.home,'')
    END AS opponent,
    CASE
      WHEN UPPER(TRIM(m.home)) = $clubNorm THEN 'home'
      ELSE 'away'
    END AS venue
  FROM matches m
  $whereSql
...
foreach ($matches as $m) {
  ...
  $sl = $gf . ' x ' . $ga;
  $scorelines[str_replace(' ', '', $sl)] = ($scorelines[str_replace(' ', '', $sl)] ?? 0) + 1;
  ...
  if ($gf>$ga) $opponents[$key]['wins']++;
  elseif ($gf<$ga) $opponents[$key]['losses']++;
  else $opponents[$key]['draws']++;
  ...
  $stadiums[$key]['played']++;
  if ($gf>$ga) $stadiums[$key]['wins']++;
  elseif ($gf<$ga) $stadiums[$key]['losses']++;
  else $stadiums[$key]['draws']++;
  ...
  $key = normKey($label);
  if (!isset($referees[$key])) $referees[$key] = ['label'=>$label,'played'=>0,'wins'=>0,'draws'=>0,'losses'=>0];
  $referees[$key]['played']++;
  if ($gf>$ga) $referees[$key]['wins']++;
  elseif ($gf<$ga) $referees[$key]['losses']++;
  else $referees[$key]['draws']++;
...
// Vitórias com mais gols marcados
$tmp = [];
foreach ($matches as $m) {
  $gf = (int)$m['gf']; $ga = (int)$m['ga'];
  if ($gf > $ga) {
...
$topConsecutiveGames = [];
if ($matches) {
  $matchIds = array_map(static fn(array $r): int => (int)$r['id'], $matches);
  $matchIdList = implode(',', array_map('intval', $matchIds));

  $sqlPlays = "
    SELECT DISTINCT part.match_id, part.player_id
    FROM (
      $realParticipationSql
    ) part
  ...
  foreach ($matchIds as $mid) {
    ...
    foreach (array_keys($set) as $pid) {
      $cur[$pid] = ($cur[$pid] ?? 0) + 1;
      $best[$pid] = max($best[$pid] ?? 0, $cur[$pid]);
    }
  }

  if ($best) {
    $playerIds = array_map('intval', array_keys($best));
    $in = implode(',', $playerIds);

    $sqlNames = "SELECT id, name FROM players WHERE id IN ($in) " . ($playersHasUserId ? "AND user_id = :user_id" : "");
    $st = $pdo->prepare($sqlNames);
    $nameBind = [];
    if ($playersHasUserId) $nameBind[':user_id'] = $userId;
    $st->execute($nameBind);
    $names = [];
    foreach (($st->fetchAll(PDO::FETCH_ASSOC) ?: []) as $r) {
    ...
    foreach ($best as $pid => $streak) {
      $topConsecutiveGames[] = [
        'player_name' => $names[(int)$pid] ?? ('#'.$pid),
        'streak'      => (int)$streak,
      ];
    }

    usort($topConsecutiveGames, fn($a,$b)=> ($b['streak'] <=> $a['streak']) ?: strcmp($a['player_name'],$b['player_name']));
    $topConsecutiveGames = array_slice($topConsecutiveGames, 0, 10);
  }
}
  ...
  if (in_array('rating', $cols, true)) {
    $sql = "
      SELECT
        ...
        ROUND(AVG(CAST(mps.rating AS REAL)), 2) AS avg_rating,
      ...
      " . ($whereSql ? "AND" : "WHERE") . " $isClubInMatch
        " . ($matchPlayerStatsHasUserId ? "AND mps.user_id = :user_id " : "") . "
        " . ($hasClubCol ? "AND UPPER(TRIM(mps.club_name)) = $clubNorm" : "") . "
        AND CAST(COALESCE(mps.rating, 0) AS REAL) > 0
      GROUP BY mps.player_id
      HAVING COUNT(DISTINCT mps.match_id) > 0
      ORDER BY avg_rating DESC, games DESC, player_name ASC
      LIMIT 10
    ";
    $st = $pdo->prepare($sql);
    $st->execute($bind);
    $bestAverages = $st->fetchAll(PDO::FETCH_ASSOC) ?: [];
    foreach ($bestAverages as &$row) {
      $row['avg_rating'] = number_format((float)($row['avg_rating'] ?? 0), 2, ',', '.');
      $row['games'] = (int)($row['games'] ?? 0);
    }
    unset($row);
    $bestAveragesAvailable = true;
  }
}
...
echo '<div class="col-12 col-lg-4">';
echo '  <div class="card-soft p-3">';
echo '    <h5 class="mb-2">Ataque & Defesa</h5>';
...
echo '    <div><b>Invicto:</b> '.(int)$streaks['unbeaten']['len'].' <span class="text-muted">('.h((string)($streaks['unbeaten']['start'] ?? '-')).' → '.h((string)($streaks['unbeaten']['end'] ?? '-')).')</span></div>';
...
echo '<div class="row g-3 mb-3">';
echo '<div class="col-12 col-lg-6"><div class="card-soft p-3"><h5 class="mb-2">Top 10 goleiros com clean sheet</h5>';
render_table(['Goleiro','Clean sheets'], $topCleanSheets, ['player_name','clean_sheets'], 2);
echo '</div></div>';
echo '<div class="col-12 col-lg-6"><div class="card-soft p-3"><h5 class="mb-2">Top 100 jogadores com mais jogos</h5>';
render_table_scroll(['Jogador','Jogos'], $topGames100, ['player_name','games'], 2, 340);
echo '</div></div>';
echo '</div>';

echo '<div class="row g-3 mb-3">';
echo '<div class="col-12 col-lg-6"><div class="card-soft p-3 h-100">';
...
echo '</div></div>';
echo '<div class="col-12 col-lg-6"><div class="card-soft p-3 h-100">';
echo '<h5 class="mb-2">Melhores Médias</h5>';
if ($bestAveragesAvailable) {
  render_table_scroll(['Jogador','Média','Jogos'], $bestAverages, ['player_name','avg_rating','games'], 3, 340);
} else {
  echo '<div class="text-muted">Sem dados de notas disponíveis na tabela <code>match_player_stats</code>.</div>';
}
echo '</div></div>';
echo '</div>';
