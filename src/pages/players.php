...
function players_table_info(PDO $pdo, string $table): array
{
  $out = [];
  try {
...
if (isset($_GET['retire'])) {
  ...
  if ($p) {
    ...
    $transferCols = players_table_info($pdo, 'transfers');
    $transferData = [];

    if (isset($transferCols['user_id']))               $transferData['user_id'] = $userId;
    if (isset($transferCols['season']))                $transferData['season'] = $season;
    if (isset($transferCols['type']))                  $transferData['type'] = 'APOSENTADORIA';
    if (isset($transferCols['player_id']))             $transferData['player_id'] = (int)$p['id'];
    if (isset($transferCols['athlete_name']))          $transferData['athlete_name'] = (string)$p['name'];
    if (isset($transferCols['club_origin']))           $transferData['club_origin'] = app_club();
    if (isset($transferCols['club_destination']))      $transferData['club_destination'] = 'APOSENTADORIA';
    if (isset($transferCols['value']))                 $transferData['value'] = 0;
    if (isset($transferCols['value_eur_cents']))       $transferData['value_eur_cents'] = 0;
    if (isset($transferCols['term']))                  $transferData['term'] = '';
    if (isset($transferCols['grade']))                 $transferData['grade'] = '';
    if (isset($transferCols['extra_player_name']))     $transferData['extra_player_name'] = '';
    if (isset($transferCols['shirt_number_assigned'])) $transferData['shirt_number_assigned'] = null;
    if (isset($transferCols['transaction_date']))      $transferData['transaction_date'] = $today;
    if (isset($transferCols['negotiation_date']))      $transferData['negotiation_date'] = $today;
    if (isset($transferCols['effective_date']))        $transferData['effective_date'] = $today;
    if (isset($transferCols['notes']))                 $transferData['notes'] = 'Aposentado via players.php';
    if (isset($transferCols['player_yd']))             $transferData['player_yd'] = null;

    if ($transferData) {
      $cols = array_keys($transferData);
      $placeholders = array_map(fn($c) => ':' . $c, $cols);

      q(
        $pdo,
        "INSERT INTO transfers (" . implode(', ', $cols) . ")
         VALUES (" . implode(', ', $placeholders) . ")",
        array_combine($placeholders, array_values($transferData))
      );
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $shirtChangeDate = trim((string)($_POST['shirt_change_date'] ?? ''));
  ...
  if ($err === '') {
    if ($id > 0) {
      ...
      $oldShirt = $before['shirt_number'] ?? null;
      $shirtChanged = ($oldShirt === null || $oldShirt === '')
        ? ($shirt !== null)
        : ((int)$oldShirt !== (int)$shirt);

      if ($shirtChanged) {
        if ($shirtChangeDate === '') {
          $err = 'Informe a data da alteração da numeração.';
        } elseif (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $shirtChangeDate)) {
          $err = 'Informe uma data válida para a alteração da numeração.';
        }
      }
      ...
      if ($err === '') {
        ...
        if ($shirtChanged) {
          pm_sync_player_shirt_history(
            ...
            $shirt,
            $shirtChangeDate,
            'Alteração manual em players.php'
          );
        }
      ...
      pm_sync_player_shirt_history($pdo, $userId, $newPlayerId, $shirt, date('Y-m-d'), 'Cadastro inicial em players.php');
  ...
  $edit = [
    ...
    'secondary_positions' => $sec,
    'is_active' => $active,
    'shirt_change_date' => $shirtChangeDate,
  ];
}
...
if ($edit && isset($edit['id']) && (int)$edit['id'] > 0) {
  echo '<div class="mb-3">';
  echo '<label class="form-label">Data da alteração da numeração</label>';
  echo '<input type="date" class="form-control" name="shirt_change_date" value="' . h((string)($edit['shirt_change_date'] ?? '')) . '">';
  echo '<div class="form-text">Preencha esta data apenas quando alterar o número da camisa do atleta.</div>';
  echo '</div>';
}
