...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string)($_POST['action'] ?? '') === 'reset_save') {
  try {
    ...
    |--------------------------------------------------------------------------
    | Ordem correta respeitando todas as FKs do schema:
    |
    | 1. player_shirt_history  → FK players(id)
    | 2. match_player_stats    → FK matches(id) + players(id)
    | 3. match_substitutions   → user_id direto
    | 4. substitutions         → FK matches(id) + players(id)
    | 5. match_players         → FK matches(id) + players(id)
    | 6. opponent_match_player_stats → user_id direto
    | 7. opponent_players      → user_id direto
    | 8. matches               → tabela pai (apagar após dependentes)
    | 9. lineup_template_slots → FK lineup_templates(id) + players(id)
    | 10. lineup_templates     → tabela pai dos slots
    | 11. injuries             → FK players(id)
    | 12. players              → tabela pai (apagar após todos dependentes)
    | 13. academy_dismissed    → user_id direto
    | 14. academy_players      → user_id direto
    | 15. transfers            → user_id direto
    | 16. trophies             → user_id direto
    |--------------------------------------------------------------------------
    */

    // Tabelas com user_id direto — deletar via user_id
    $tablesByUserId = [
      'player_shirt_history',
      'match_player_stats',
      'match_substitutions',
      'substitutions',
      'match_players',
    ...
    foreach ($tablesByUserId as $table) {
      // Verifica se a tabela possui coluna user_id antes de deletar
      $hasUserIdCol = false;
      try {
        $pragma = $pdo->query("PRAGMA table_info({$table})")->fetchAll(PDO::FETCH_ASSOC);
      ...
      if ($hasUserIdCol) {
        q($pdo, "DELETE FROM {$table} WHERE user_id = :user_id", [':user_id' => $userId]);
      }
    }
    ...
    // Registrar erro em arquivo de log (nunca exibir na tela)
    $logDir  = __DIR__ . '/../../logs';
    $logFile = $logDir . '/reset_save_errors.log';
    if (!is_dir($logDir)) {
      @mkdir($logDir, 0755, true);
    }
    $logLine = sprintf(
      "[%s] user_id=%d | %s: %s | File: %s:%d\n",
      date('Y-m-d H:i:s'),
      $userId,
      get_class($e),
      $e->getMessage(),
