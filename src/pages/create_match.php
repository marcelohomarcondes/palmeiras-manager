...
$kits      = ['Home','Away','Alternativo 1','Alternativo 2','Alternativo 3'];
...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST' && $editId > 0) {
  ...
  for ($i=0;$i<9;$i++){
    ...
    $_POST["pal_pos_bench_$i"] = 'RES';
    $_POST["opp_name_bench_$i"] = $_POST["opp_name_bench_$i"] ?? '';
    $_POST["opp_pos_bench_$i"]  = 'RES';
  }

  $mpCols = table_info($pdo, 'match_players');
  ...
  if (isset($mpCols['club_name'])) {
    $mpSelect[] = 'club_name';
  }
  if (isset($mpCols['player_type'])) {
    $mpSelect[] = 'player_type';
  }
  if (isset($mpCols['opponent_name'])) {
    $mpSelect[] = 'opponent_name';
  }

  $mpSql = "
    SELECT " . implode(', ', $mpSelect) . "
    FROM match_players
...
$homeNorm = strtoupper(trim($homeName));
$awayNorm = strtoupper(trim($awayName));
...
foreach ($mpr as $r) {
  ...
  $pos = strtoupper(trim((string)($r['position'] ?? '')));
  if ($roleL === 'bench') {
    $pos = 'RES';
  } elseif ($pos === 'RES') {
    $pos = '';
  ...
  $playerType = strtoupper(trim((string)($r['player_type'] ?? '')));
  ...
  if (!empty($r['player_id'])) {
    $isPalRow = true;
  } elseif (!empty($r['opponent_player_id']) || trim((string)($r['opponent_name'] ?? '')) !== '') {
    $isOppRow = true;
  }

  if (!$isPalRow && !$isOppRow) {
    if ($clubName !== '') {
      if ($clubName === strtoupper(trim($club))) {
        $isPalRow = true;
      } elseif ($clubName === $homeNorm || $clubName === $awayNorm) {
        $isOppRow = true;
      }
    } elseif ($playerType === $palType) {
      $isPalRow = true;
    } elseif ($playerType === $oppType) {
      $isOppRow = true;
    }
  }

  if ($isPalRow && !empty($r['player_id'])) {
  ...
  if (table_exists($pdo, 'match_penalties')) {
    $penSql = "SELECT * FROM match_penalties WHERE match_id = ? ";
    $penParams = [$editId];
    if (has_user_id_col($pdo, 'match_penalties')) {
      $penSql .= " AND user_id = ? ";
      $penParams[] = $userId;
    }
    $penSql .= " ORDER BY team ASC, order_number ASC ";
    $penalties = q($pdo, $penSql, $penParams)->fetchAll(PDO::FETCH_ASSOC) ?: [];

    $palPenIdx = 0;
    $oppPenIdx = 0;
    foreach ($penalties as $pen) {
      $team = strtoupper((string)($pen['team'] ?? ''));
      $name = (string)($pen['player_name'] ?? '');
      $scored = (string)($pen['scored'] ?? '0');

      if ($team === $palType && $palPenIdx < 10) {
        // Resolver nome para player_id para o select
        $pidRow = q($pdo, "SELECT id FROM players WHERE name = ? COLLATE NOCASE LIMIT 1", [$name])->fetch(PDO::FETCH_ASSOC);
        $_POST["pal_pen_name_$palPenIdx"] = $pidRow ? (string)$pidRow['id'] : $name;
        $_POST["pal_pen_result_$palPenIdx"] = $scored;
        $palPenIdx++;
      } elseif ($team === $oppType && $oppPenIdx < 10) {
        $_POST["opp_pen_name_$oppPenIdx"] = $name;
        $_POST["opp_pen_result_$oppPenIdx"] = $scored;
        $oppPenIdx++;
      }
    }
  }
...
$palShirtById = [];
foreach ($palPlayers as $pp) {
  $pid = (int)($pp['id'] ?? 0);
  if ($pid > 0) {
    $palShirtById[$pid] = (($pp['shirt_number'] ?? '') === '' ? null : (int)$pp['shirt_number']);
  }
}
...
function render_table_create(
  ...
  echo '<thead><tr class="text-center">
    ...
    <th style="min-width:280px;">Atleta</th>
    <th style="width:52px;">G</th>
    <th style="width:52px;">A</th>
    <th style="width:52px;">GC</th>
    <th style="width:52px;">CA</th>
    <th style="width:52px;">CV</th>
    <th style="width:60px;">Nota</th>
    <th style="width:48px;">MVP</th>
  </tr></thead><tbody>';
  ...
  for ($i = 0; $i < $maxRows; $i++) {
    ...
    $small = 'class="form-control form-control-sm text-center px-1" style="width:46px;min-width:46px;max-width:46px;height:40px;margin:0 auto;border-radius:14px;"';
    $note  = 'class="form-control form-control-sm text-center px-1" style="width:52px;min-width:52px;max-width:52px;height:40px;margin:0 auto;border-radius:14px;"';

    $posVal = strtoupper(postv($posKey));

    echo '<tr class="text-center">';
    if ($role === 'starter') {
      if ($posVal === 'RES') $posVal = '';
      echo '<td class="px-2">
        <select class="form-select form-select-sm text-center px-1" name="'.h($posKey).'" style="width:52px;min-width:52px;max-width:52px;margin:0 auto;">
          <option value=""></option>'.select_options($starterPositions, $posVal).'
        </select>
      </td>';
    } else {
      echo '<td class="px-2">';
      echo '<input type="hidden" name="'.h($posKey).'" value="RES">';
      echo '<input class="form-control form-control-sm text-center px-1" value="RES" disabled style="width:52px;min-width:52px;max-width:52px;margin:0 auto;">';
      echo '</td>';
    }

    echo '<td class="text-start px-2">';
    if ($isPal) {
    ...
    echo '<td class="px-1"><input '.$small.' type="number" min="0" name="'.h($gKey).'" value="'.h(postv($gKey)).'"></td>';
    echo '<td class="px-1"><input '.$small.' type="number" min="0" name="'.h($aKey).'" value="'.h(postv($aKey)).'"></td>';
    echo '<td class="px-1"><input '.$small.' type="number" min="0" name="'.h($gcKey).'" value="'.h(postv($gcKey)).'"></td>';
    echo '<td class="px-1"><input '.$small.' type="number" min="0" name="'.h($caKey).'" value="'.h(postv($caKey)).'"></td>';
    echo '<td class="px-1"><input '.$small.' type="number" min="0" name="'.h($cvKey).'" value="'.h(postv($cvKey)).'"></td>';
    echo '<td class="px-1"><input '.$note.' type="text" name="'.h($rtKey).'" value="'.h(postv($rtKey)).'"></td>';
    echo '<td class="px-1"><input type="radio" name="mvp" value="'.h($mvpVal).'" '.$checkedMvp.'></td>';
    echo '</tr>';
  }
...
function render_penalties_block(string $prefix, string $title): void {
  echo '<h6 class="mt-3">'.h($title).'</h6>';
  echo '<div class="table-responsive">';
  echo '<table class="table table-sm align-middle mb-0">';
  echo '<thead><tr class="text-center">
  ...
  for ($i=0; $i<10; $i++) {
    $kName = $prefix.'_pen_name_'.$i;
    $kResult = $prefix.'_pen_result_'.$i;

    echo '<tr class="text-center">';
    echo '<td>'.($i+1).'</td>';
    echo '<td><select class="form-select form-select-sm w-100 pen-player" data-prefix="'.h($prefix).'" data-selected="'.h(postv($kName)).'" name="'.h($kName).'"><option value="">-- Batedor --</option></select></td>';
    echo '<td>
      <div class="btn-group btn-group-sm w-100" role="group">
        <input type="radio" class="btn-check" name="'.h($kResult).'" id="'.h($prefix.'_pen_scored_'.$i).'" value="1" '.(postv($kResult)==='1' ? 'checked' : '').'>
        <label class="btn btn-outline-success" for="'.h($prefix.'_pen_scored_'.$i).'">✓ Gol</label>
        
        <input type="radio" class="btn-check" name="'.h($kResult).'" id="'.h($prefix.'_pen_missed_'.$i).'" value="0" '.(postv($kResult)==='0' ? 'checked' : '').'>
        <label class="btn btn-outline-danger" for="'.h($prefix.'_pen_missed_'.$i).'">✗ Perdeu</label>
      </div>
    </td>';
    echo '</tr>';
  }
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  ...
  if ($action === 'apply_template') {
    ...
    try {
      ...
      for ($i=0;$i<$MAX_BENCH;$i++){ $_POST["pal_pid_bench_$i"]='0'; $_POST["pal_pos_bench_$i"]='RES'; }
  ...
  for ($i=0;$i<$MAX_STARTERS;$i++) {
    ...
    $pos = strtoupper(postv("pal_pos_starter_$i"));
  ...
  for ($i=0;$i<$MAX_STARTERS;$i++) {
    ...
    $pos  = strtoupper(postv("opp_pos_starter_$i"));
  ...
  // Validate exactly 11 starters per team
  $palStarterCount = count(array_filter($palRows, fn($r) => $r['role'] === 'STARTER'));
  $oppStarterCount = count(array_filter($oppRows, fn($r) => $r['role'] === 'STARTER'));
  if ($palStarterCount !== $MAX_STARTERS) redirect(current_match_form_url('err=pal_starters_required'));
  if ($oppStarterCount !== $MAX_STARTERS) redirect(current_match_form_url('err=opp_starters_required'));

  foreach ($palRows as $r) {
    if ($r['role'] === 'STARTER') {
      if ((string)$r['position'] === 'RES') redirect(current_match_form_url('err=starter_pos_res'));
      if (!in_array((string)$r['position'], $starterPositions, true)) redirect(current_match_form_url('err=starter_pos_invalid'));
    } elseif ((string)$r['position'] !== 'RES') {
      redirect(current_match_form_url('err=bench_pos_not_res'));
    }
  }
  ...
  try {
    ...
    foreach ($palRows as $r) {
      ...
      if (isset($mpInfo['player_id'])) $data['player_id'] = $playerId;
      if (isset($mpInfo['opponent_player_id'])) $data['opponent_player_id'] = null;
      if (isset($mpInfo['opponent_name'])) $data['opponent_name'] = $name;
    ...
    // Atualizar flag 'entered' para reservas que entraram via substituição
    // Palmeiras bench players who came on
    $subInPalIds = array_filter(array_map(fn($s) => (int)$s['in'], $subsPal));
    if ($subInPalIds) {
      $inPh = implode(',', array_fill(0, count($subInPalIds), '?'));
      $params = array_merge([$matchId], $subInPalIds);
      q($pdo, "UPDATE match_players SET entered = 1 WHERE match_id = ? AND player_id IN ($inPh) AND role = 'BENCH'", $params);
    }
    // Opponent bench players who came on
    $subInOppIds = [];
    foreach ($subsOpp as $s) {
      $inKey = strtolower(trim((string)$s['in_name']));
      $oid = (int)($oppIdByName[$inKey] ?? 0);
      if ($oid > 0) $subInOppIds[] = $oid;
    }
    if ($subInOppIds) {
      $inPh = implode(',', array_fill(0, count($subInOppIds), '?'));
      $params = array_merge([$matchId], $subInOppIds);
      q($pdo, "UPDATE match_players SET entered = 1 WHERE match_id = ? AND opponent_player_id IN ($inPh) AND role = 'BENCH'", $params);
    }

    // Salvar pênaltis
    if (table_exists($pdo, 'match_penalties')) {
      $params = [$matchId];
      $suffix = has_user_id_col($pdo, 'match_penalties') ? ' AND user_id = ?' : '';
      if ($suffix !== '') $params[] = $userId;
      q($pdo, "DELETE FROM match_penalties WHERE match_id = ?{$suffix}", $params);

      $penaltiesPal = [];
      $penaltiesOpp = [];

      // Coletar pênaltis do Palmeiras (select envia player_id)
      // Construir mapa id->name para resolver
      $palPenPlayerMap = [];
      {
        $palPenIds = [];
        for ($i=0; $i<10; $i++) {
          $v = trim(postv("pal_pen_name_$i"));
          if ($v !== '' && ctype_digit($v)) $palPenIds[] = (int)$v;
        }
        if ($palPenIds) {
          $inPh = implode(',', array_fill(0, count($palPenIds), '?'));
          $rows = q($pdo, "SELECT id, name FROM players WHERE id IN ($inPh)", $palPenIds);
          foreach ($rows as $r) $palPenPlayerMap[(int)$r['id']] = $r['name'];
        }
      }
      for ($i=0; $i<10; $i++) {
        $val = trim(postv("pal_pen_name_$i"));
        $result = postv("pal_pen_result_$i");
        if ($val === '' || $result === '') continue;
        // Resolver: se for numérico, buscar nome no mapa; senão usar como texto
        $name = (ctype_digit($val) && isset($palPenPlayerMap[(int)$val]))
              ? $palPenPlayerMap[(int)$val]
              : $val;
        $penaltiesPal[] = [
          'name' => $name,
          'scored' => (int)$result,
      ...
      // Coletar pênaltis do adversário
      for ($i=0; $i<10; $i++) {
        $name = trim(postv("opp_pen_name_$i"));
        $result = postv("opp_pen_result_$i");
        if ($name !== '' && $result !== '') {
          $penaltiesOpp[] = [
            'name' => $name,
            'scored' => (int)$result,
      ...
      // Salvar pênaltis do Palmeiras
      foreach ($penaltiesPal as $p) {
        $data = [];
        add_user_id_if_exists($pdo, 'match_penalties', $data, $userId);
        $data['match_id'] = $matchId;
        $data['team'] = $palType;
        $data['player_name'] = $p['name'];
        $data['order_number'] = $p['order'];
        $data['scored'] = $p['scored'];
        insert_dynamic($pdo, 'match_penalties', $data);
      }

      // Salvar pênaltis do adversário
      foreach ($penaltiesOpp as $p) {
        $data = [];
        add_user_id_if_exists($pdo, 'match_penalties', $data, $userId);
        $data['match_id'] = $matchId;
        $data['team'] = $oppType;
...
if ($err !== '') {
  $errMessages = [
    'roster_required' => 'Elenco obrigatório: preencha ao menos um jogador por time.',
    'pal_starters_required' => 'O Palmeiras precisa ter exatamente 11 titulares.',
    'opp_starters_required' => 'O adversário precisa ter exatamente 11 titulares.',
    'dup_player' => 'Jogador duplicado detectado no elenco.',
    'subs_incomplete' => 'Substituição incompleta: preencha quem entra e quem sai.',
    'subs_same' => 'Substituição inválida: jogador não pode substituir a si mesmo.',
    'subs_out_not_starter' => 'Jogador que sai na substituição não é titular.',
    'subs_in_not_bench' => 'Jogador que entra na substituição não é reserva.',
    'palmeiras_only' => 'O Palmeiras precisa ser um dos times da partida.',
    'starter_pos_res' => 'Titulares não podem ter posição RES.',
    'starter_pos_invalid' => 'Titulares devem usar uma posição válida (sem RES).',
    'bench_pos_not_res' => 'Reservas devem ter posição RES.',
    'exception' => 'Erro inesperado ao salvar a partida.',
  ];
  $errText = $errMessages[$err] ?? $err;
  echo '<div class="alert alert-danger card-soft">'.h($errText).'</div>';
}
...
render_table_create(true, 'starter', $MAX_STARTERS, $starterPositions, $palPlayers, $mvpSelected);
render_table_create(true, 'bench',   $MAX_BENCH,    $starterPositions, $palPlayers, $mvpSelected);
render_subs_block('pal', 'Substituições (até 5)');
render_penalties_block('pal', 'Pênaltis (até 10)');
...
render_table_create(false, 'starter', $MAX_STARTERS, $starterPositions, $palPlayers, $mvpSelected);
render_table_create(false, 'bench',   $MAX_BENCH,    $starterPositions, $palPlayers, $mvpSelected);
render_subs_block('opp', 'Substituições (até 5)');
render_penalties_block('opp', 'Pênaltis (até 10)');
echo '</div></div>';
...
(function(){
  ...
  function refresh(prefix){
    ...
    /* Pênaltis – qualquer jogador (titulares + reservas) pode bater */
    var penOpts = uniqByKey(outOpts.concat(inOpts), function(x){ return String(x.value).toLowerCase(); });
    penOpts.sort(function(a,b){ return a.label.localeCompare(b.label); });
    document.querySelectorAll("select.pen-player[data-prefix=\\"" + prefix + "\\"]").forEach(function(sel){
      setOptions(sel, penOpts);
    });
  }
