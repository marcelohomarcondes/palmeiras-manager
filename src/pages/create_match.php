...
/**
 * LOG simples do sistema (Windows)
 * Caminho fixo: D:\Projetos\palmeiras_manager\logs\app.log
 */
...
// Temporadas (por padrão: ano atual +/- alguns)
$curY = (int)date('Y');
$seasons = [];
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  ...
  // ==========================================================
  // VALIDAÇÃO: NÃO PERMITIR O MESMO JOGADOR DO PALMEIRAS 2x
  // (Titulares + Reservas). NÃO se aplica ao adversário.
  // ==========================================================
  $palSeen = [];
  ...
  try {
    ...
    // Opponent players (cria em players se não existir)
...
if ($err === 'invalid') {
  ...
  echo '<div class="alert alert-danger card-soft">Falha ao cadastrar a partida. Verifique o log em <code>D:\\Projetos\\palmeiras_manager\\logs\\app.log</code> e tente novamente.</div>';
} elseif ($msg === 'saved') {
...
.pm-roster-table select, .pm-roster-table input{
  width: 100%; height: 40px; border-radius: 14px; padding: 0 12px;
}
.pm-roster-table td:first-child select, .pm-roster-table td:first-child input{
  padding-left: 14px; padding-right: 34px;
}
...
echo '<select class="form-select" name="season" required><option value="">-- selecione --</option>'.select_options($seasons, fval('season')).'</select>';
echo '</div>';

echo '<div class="col-12 col-md-4">';
echo '<label class="form-label">Campeonato</label>';
echo '<select class="form-select" name="competition" required><option value="">-- selecione --</option>'.select_options($competitions, fval('competition')).'</select>';
echo '</div>';
...
echo '<select class="form-select" name="kit_used" required><option value="">-- selecione --</option>'.select_options($kits, fval('kit_used')).'</select>';
echo '</div>';

echo '<div class="col-12 col-md-2">';
echo '<label class="form-label">Clima</label>';
echo '<select class="form-select" name="weather" required><option value="">-- selecione --</option>'.select_options($weathers, fval('weather')).'</select>';
echo '</div>';

echo '<div class="col-12 col-md-4">';
echo '<label class="form-label">Mandante</label>';
echo '<input class="form-control" name="home" value="'.h(fval('home')).'" required>';
echo '</div>';

echo '<div class="col-12 col-md-4">';
echo '<label class="form-label">Visitante</label>';
echo '<input class="form-control" name="away" value="'.h(fval('away')).'" required>';
echo '</div>';
...
echo '<div class="text-muted mb-3">Cadastre titulares e reservas do '.h($club).' e do adversário. Para o adversário, você pode digitar o nome e nós salvaremos como atleta do clube informado.</div>';

$cg = '';

echo '<h6 class="mt-3">'.h($club).'</h6>';
...
echo '<h6 class="mt-4">Adversário</h6>';
echo '<div class="mb-2 fw-bold">Titulares</div>';
echo '<div class="table-responsive">';
echo '<table class="table table-sm pm-roster-table align-middle mb-3">';
echo '<colgroup>
