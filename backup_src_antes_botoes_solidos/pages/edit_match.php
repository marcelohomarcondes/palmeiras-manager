...
/* =========================================================
   EDIT MODE (pré-preenchimento ao abrir com ?page=edit_match&id=XX)
   - Não altera front-end: apenas injeta valores em $_POST/$_GET para o form já existente
   ========================================================= */
$editId = (int)($_GET['id'] ?? 0);


// edit_match.php exige um id de partida na URL (ou match_id no POST)
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST' && $editId <= 0) {
  redirect('/?page=matches&err=missing_id');
}
...
$palPlayers = [];
try {
  // Em edição: incluir ATIVOS + atletas já vinculados a esta partida (mesmo que hoje estejam transferidos/inativos)
  if ($editId > 0 && table_exists($pdo, 'match_players')) {
    $palPlayers = q($pdo, "
      ...
      ORDER BY name
    ", [$club, $editId, $club])->fetchAll(PDO::FETCH_ASSOC) ?: [];
  } else {
    // fallback (não deve acontecer no edit_match)
    $palPlayers = q($pdo, "
      SELECT id, name, shirt_number
      FROM players
      WHERE is_active = 1
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  ...
  if ($action === 'apply_template') {
    ...
    if ($tplId <= 0) { redirect('?page=edit_match&err=tpl'); }

    try {

      $tplSchema = detect_templates_schema($pdo);
      if (!$tplSchema['slotTable']) { redirect('?page=edit_match&err=tpl_slots'); }
      ...
      redirect('?page=edit_match&err=tpl');
    }
  }
  ...
  if (!$isHomePal && !$isAwayPal) redirect('?page=edit_match&err=palmeiras_only');
  ...
  if (count($palRows) < 1 || count($oppRows) < 1) redirect('?page=edit_match&err=roster_required');

  // evita repetição no Palmeiras
  $seen = [];
  foreach ($palRows as $r) {
    $pid = (int)$r['player_id'];
    if (isset($seen[$pid])) redirect('?page=edit_match&err=dup_player');
    $seen[$pid]=true;
  }
  ...
  for ($i=0; $i<5; $i++) {
    ...
    if ($outId === 0 || $inId === 0) redirect('?page=edit_match&err=subs_incomplete');
    if ($outId === $inId) redirect('?page=edit_match&err=subs_same');

    if (!isset($palStarters[$outId])) redirect('?page=edit_match&err=subs_out_not_starter');
    if (!isset($palBench[$inId]))     redirect('?page=edit_match&err=subs_in_not_bench');
  ...
  for ($i=0; $i<5; $i++) {
    ...
    if ($outNm === '' || $inNm === '') redirect('?page=edit_match&err=subs_incomplete');
    if (strcasecmp($outNm, $inNm) === 0) redirect('?page=edit_match&err=subs_same');

    $kOut = strtolower($outNm);
    $kIn  = strtolower($inNm);
    if (!isset($oppStarters[$kOut])) redirect('?page=edit_match&err=subs_out_not_starter');
    if (!isset($oppBench[$kIn]))     redirect('?page=edit_match&err=subs_in_not_bench');
  ...
  if (!$colHome || !$colAway) redirect('?page=edit_match&err=exception');
  ...
  try {
    ...
    redirect('?page=edit_match&err=exception');
  }
}

RENDER_PAGE:

render_header('Editar Partida');
...
echo '<div class="text-end mt-3">
  <button type="submit" class="btn btnbtn-primary btn-primary">Salvar</button>
</div>';
