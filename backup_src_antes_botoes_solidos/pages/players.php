...
if ($edit) echo ' <a class="btnbtn-outline-secondary" href="/?page=players">Cancelar</a>';
echo '</form>';
...
function render_section(string $title, array $list, ?string $emptyText = null): void {
  ...
  if (!$list) {
    ...
    foreach ($list as $r) {
      ...
      echo '<a class="btn btn-sm btn-outline-danger" href="/?page=players&del=' . (int)$r['id'] . '" onclick="return confirm(\'Confirmar exclusão?\')">Excluir</a>';
      echo '</td>';
